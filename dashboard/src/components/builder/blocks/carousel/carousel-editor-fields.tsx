      ...
      <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1 scrollbar-thin">
        ...
        {item.items?.map((slide, slideIndex) => {
          ...
          return (
            ...
            >
              <div className={`flex flex-col sm:flex-row gap-3 justify-between ${isEditing ? 'sm:items-start' : 'sm:items-center'}`}>
                {/* Slide Info (Thumbnail + Details) */}
                ...
                {/* Action Toolbar (Horizontal layout, responsive position) */}
                <div className={`flex flex-row items-center gap-1.5 border-t sm:border-t-0 sm:border-l border-border pt-2 sm:pt-0 pl-0 sm:pl-2.5 mt-2 sm:mt-0 justify-end w-full sm:w-auto shrink-0 ${isEditing ? 'sm:self-start sm:mt-1' : 'sm:self-center'}`}>
                  <button
                    type="button"
